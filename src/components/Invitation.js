...
const Invitation = (props) => {
    ...
    const submitResponseEvent1 = async (e) => {
        ...
        try {
            ...
            const response = await fetch("http://luvandkrishi.com:8080/v1/family/events/" + props.familyID + "/" + 1 + "/" + numberAttendingEvent1, requestOptions)
    ...
    const submitResponseEvent2 = async (e) => {
      ...
      try {
        console.log("Hey this is event 2's handler!");
          ...
          const response = await fetch("http://luvandkrishi.com:8080/v1/family/events/" + props.familyID + "/" + 2 + "/" + numberAttendingEvent2, requestOptions)
          const rsvpData = await response.json();
          if (response.status!==200 && response.status!==400) {
              alert("Hi! Something seems to be off on our end, please email luvandkrishi.com!");
          }
  ...
  const submitResponseEvent3 = async (e) => {
    ...
    try {
      console.log("Hey this is event 3's handler!");
        ...
        const response = await fetch("http://luvandkrishi.com:8080/v1/family/events/" + props.familyID + "/" + 3 + "/" + numberAttendingEvent3, requestOptions)
        const rsvpData = await response.json();
        if (response.status!==200 && response.status!==400) {
            alert("Hi! Something seems to be off on our end, please email luvandkrishi.com!");
        }
...
const notAttending = async (e) => {
  ...
  try {
    console.log("User is not attending wedding!");
      ...
      const response = await fetch("http://luvandkrishi.com:8080/v1/family/events/decline/" + props.familyID, requestOptions)
      const rsvpData = await response.json();
      if (response.status!==200 && response.status!==400) {
          alert("Hi! Something seems to be off on our end, please email luvandkrishi.com!");
      }
